...
class Encoder(nn.Module):
    def __init__(self, scale=1):
        ...
        self.down_1 = Downsample_Vit(36 // scale, 72 // scale, 4, apply_batchnorm=False, use_residual=False, need_attention=True,attn_size=448, fp16=True, head=1)  # 448 -> 224
        self.down_2 = Downsample_Vit(72 // scale, 144 // scale, 4, need_attention=True, attn_size=224, fp16=True,head=3)  # 224 -> 112
        self.down_3 = Downsample_Vit(144 // scale, 288 // scale, 4, need_attention=True, attn_size=112, fp16=True,head=2)  # 112 -> 56
    ...
    def init_part_optimizer(self,lr=2e-6,scheduler_gamma=0.97):
        ...
        for down in self.down_stack:
                if isinstance(down, Downsample_Vit):
                    ...
                    for param in head_param:
                        print(type(param))
                        param.requires_grad=True
                        head_params.extend(param)
                    for param in hand_param:
                        param.requires_grad=True
                        hand_params.extend(param)
                    for param in global_param:
                        param.requires_grad=True
                        global_params.extend(param)


        for param in head_params:
            if not param.is_leaf:
                print(f"head param {param} requires_grad: {param.requires_grad} is not leaf!")
        head_part_optimizer = AdaBelief(head_params, lr=lr, betas=(0.5, 0.999))
        head_part_scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer=head_part_optimizer, gamma=scheduler_gamma)
...
class Generator(nn.Module):
    """
    生成器模块，基于多层下采样与上采样构建视觉转换，同时融合关键点映射和 AdaIN 融合。
    下采样部分生成 skip connection，下采样流程经过 9 个阶段，上采样部分经过 8 个阶段，
    并结合关键点映射信息对上采样分支进行调制。

    参数:
        lr_multiplier (float): 学习率倍率，默认为 0.01。
        z_dim (int): 映射网络输入维数，默认为 512。
    """
    def __init__(self, lr_multiplier=0.01, z_dim=512):
        ...
        self.up_stack = nn.ModuleList([
            self.up_1, self.up_2, self.up_3, self.up_4,
            self.up_5, self.up_6, self.up_7, self.up_8
        ])

        # 映射网络：三层全连接层
        self.mapping_network = nn.Sequential(
            FullyConnectedLayer(266, z_dim),
    ...
    def forward(self, inputs_content, inputs_style, keypoint_input_bodies, keypoint_input_faces,
        ...
        # 下采样并保存 skip 信息（取自各层未下采样前的中间特征）
        skips = []
        for down_layer in self.down_stack:
            x, x_no_down = down_layer(x)
            x_s, x_no_down_s = down_layer(x_s)
            x_skip = self.ada_in(x_no_down, x_no_down_s, False)
            x_skip = self.down_Adain(x_skip)
            skips.append(x_skip)
        # 取最后一层的 skip 特征作为上采样初始输入
        x = skips[-1]
        # 上采样过程，逐层对上采样后的输出与对应的 skip 特征进行拼接
        for idx, up_layer in enumerate(self.up_stack):
            inputs_up = (x, keypoints_info_f)
            x = up_layer(inputs_up)
            skip_feature = skips[len(self.up_stack) - 1 - idx]
            x = torch.concat([x, skip_feature], dim=1)
        # 最终上采样与输出层
...
class downsample_d(nn.Module):
    """
    下采样模块（判别器专用），通过卷积、可选的注意力模块、归一化、激活以及可选的最大池化进行下采样处理。

    参数:
        in_channels (int): 输入通道数。
        out_channels (int): 输出通道数。
        size (int): 卷积核大小，用于初始卷积转换。
        apply_batchnorm (bool): 是否添加归一化层，默认为True。
        need_down (bool): 是否进行池化下采样，默认为True。
        need_attention (bool): 是否添加注意力模块，默认为True。
        attn_size (int): 注意力模块的输入特征尺寸，默认为128。
        atten_fp16 (bool): 是否在注意力模块中使用混合精度计算，默认为False。
    """
    def __init__(self, in_channels: int, out_channels: int, size: int,
                 apply_batchnorm: bool = True, need_down: bool = True,
                 need_attention: bool = True, attn_size: int = 128, atten_fp16: bool = False):
        super().__init__()
        self.need_attention = need_attention
        layers = []
        # 卷积层：stride=1，padding使用'same'确保尺寸不变
...
class Discriminator(nn.Module):
    def __init__(self):
        super().__init__()
        out_channels = 512
        # 每一层的配置: (in_channels, out_channels, kernel_size, 参数字典)
        layers_config = [
            (6, 128, 4, dict(need_attention=False, attn_size=512, atten_fp16=True)),
            (128, 256, 4, dict(need_attention=True, attn_size=256, atten_fp16=True)),
            (256, 384, 4, dict(need_attention=True, attn_size=128, atten_fp16=True)),
            (384, out_channels, 4, dict(need_attention=True, attn_size=64, atten_fp16=True))
        ]
        self.down_layers = nn.ModuleList([
            downsample_d(in_ch, out_ch, size, **params)
            for in_ch, out_ch, size, params in layers_config
        ])
        # 构建输出块
...
class Discriminator_Hand(nn.Module):
    def __init__(self):
        super().__init__()
        out_channels = 512
        # 对于 Hand 判别器，下采样部分略有不同：部分层不使用池化（need_down=False）
        layers_config = [
            (6, 128, 4, dict(need_attention=False, attn_size=128, atten_fp16=True)),
            (128, 256, 4, dict(need_down=False, need_attention=True, attn_size=64, atten_fp16=True)),
            (256, 384, 4, dict(need_down=False, need_attention=True, attn_size=64, atten_fp16=True)),
            (384, out_channels, 4, dict(need_attention=True, attn_size=64, atten_fp16=True))
        ]
        self.down_layers = nn.ModuleList([
            downsample_d(in_ch, out_ch, size, **params)
...
class Discriminator_Head(nn.Module):
    def __init__(self):
        super().__init__()
        out_channels = 512
        # Discriminator_Head 的配置与 Hand 判别器类似
        layers_config = [
            (6, 128, 4, dict(need_attention=False, attn_size=128, atten_fp16=True)),
            (128, 256, 4, dict(need_down=False, need_attention=True, attn_size=64, atten_fp16=True)),
            (256, 384, 4, dict(need_down=False, need_attention=True, attn_size=64, atten_fp16=True)),
...
down = Downsample_Vit(channel, channel * 2, 4, apply_batchnorm=True, use_residual=True, need_attention=True,
